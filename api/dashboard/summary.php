...
try {
    ...
    // 1. Today's Stats
    $stmtToday = $pdo->query("SELECT SUM(grand_total) as revenue, COUNT(*) as count FROM sales WHERE DATE(created_at) = CURDATE() AND status = 'Paid'");
    $today = $stmtToday->fetch();
    $stats['today_revenue'] = (float)($today['revenue'] ?? 0);
    $stats['today_count'] = (int)($today['count'] ?? 0);

    // 2. Monthly Revenue
    $stmtMonth = $pdo->query("SELECT SUM(grand_total) as revenue FROM sales WHERE MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE()) AND status = 'Paid'");
    ...
    $stmtChart = $pdo->query("
        ...
        LEFT JOIN sales s ON DATE(s.created_at) = date_table.date AND s.status = 'Paid'
